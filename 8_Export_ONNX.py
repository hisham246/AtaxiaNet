...
cwd = os.getcwd()
logger_path = cwd + '\\Validation\\Best_Model\\Logger\\logs'
model_path = cwd + '\\Validation\\Best_Model\\Trained Model\\gait_net_hyp_4.pth'
train_files_path = cwd + '\\Validation\\Best_Model\\Train\\'
test_files_path = cwd + '\\Validation\\Best_Model\\Test\\'
results_path = cwd + '\\Validation\\Best_Model\\Results\\'
best_model_path = cwd + '\\Validation\\Best_Model\\Trained Model\\gait_net.onnx'
...
dummy_input = test_dataset[0]['sequence'].unsqueeze(dim=0)
...
torch.onnx.export(
    ...
    input_names=['Gait Features'],   # the model's input names
    output_names=['Gait Type'],  # the model's output names
