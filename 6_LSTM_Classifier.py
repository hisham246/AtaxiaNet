...
cwd = os.getcwd()
logger_path = cwd + '\\Validation\\Val_1\\Logger\\logs'
model_path = cwd + '\\Validation\\Without_Augmentation\\Trained Model\\gait_net.pth'
train_files_path = cwd + '\\Validation\\Val_5\\Train\\'
test_files_path = cwd + '\\Validation\\Val_5\\Test\\'
results_path = cwd + '\\Validation\\Without_Augmentation\\Results\\'
...
train_sequences = dataset(data_list(train_files_path, train_filenames)[0],
                          data_list(train_files_path, train_filenames)[1]
                          )[0]
...
for i in train_sequences:
    if i[1] == 0:
        ataxia_labels_train.append(i[1])
    else:
        normal_labels_train.append(i[1])
...
print(len(ataxia_labels_train), len(normal_labels_train))
...
checkpoint_callback = ModelCheckpoint(
    dirpath='./Validation/Best_Model/Logger/checkpoints',
    every_n_epochs=1,
    save_top_k=-1,
    verbose=True
)

logger = TensorBoardLogger(logger_path, name='Gait Type')
...
checkpoints_path = cwd + '\\Validation\\Best_Model\\Logger\\logs\\Gait Type\\checkpoints\\'

trained_model = GaitPredictor.load_from_checkpoint(
    checkpoint_path=checkpoints_path + 'best_checkpoint.ckpt',
    n_features=len(FeatureColumns),
    n_classes=len(label_encoder.classes_)
)
...
Report.to_csv(results_path + 'Classification_Report_Hyp_4.csv')
...
def show_confusion_matrix(confusion_matrix):
    ...
    hmap.xaxis.set_ticklabels(hmap.xaxis.get_ticklabels(), rotation=0, ha='right', fontsize=16)
    plt.ylabel('True Gait Type', fontsize=18)
    plt.xlabel('Predicted Gait Type', fontsize=18)
